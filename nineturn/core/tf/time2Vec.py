...
class Time2Vec(Layer):
    ...
    def build(self, input_shape):
        ...
        self.wa = self.add_weight(
            shape=(1, self.feature_size, self.kernel_size-1),
            initializer='uniform',
            trainable=True
        )
        
        self.ba = self.add_weight(
            shape=(self.feature_size, self.kernel_size-1),
            initializer='uniform',
            trainable=True
        )
    ...
    def call(self, inputs):
        ...
        bias = tf.reshape(tf.matmul(inputs, self.wb) + self.bb, [inputs.shape[0], inputs.shape[1],self.feature_size, 1]) # n w d 1 
        pro = tf.reshape(tf.einsum('abc, cde-> abcde',inputs, self.wa), [inputs.shape[0], inputs.shape[1],
            self.feature_size, -1]) 
